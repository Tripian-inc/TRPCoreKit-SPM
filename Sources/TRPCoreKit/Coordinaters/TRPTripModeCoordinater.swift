...
extension TRPTripModeCoordinater {
    ...
    public func openTripModeVC(hash:String, city: TRPCity, destinationId: String, startDay: Int = 0) {
        ...
        let viewModel = TRPTripModeViewModel(tripHash: hash, city: city, destinationId: destinationId)
        viewModel.fetchTripUseCase = tripModeUseCases
        viewModel.tripModelObserverUseCase = tripModeUseCases
        viewModel.changeDayUseCase = tripModeUseCases
        viewModel.addStepUseCase = tripModeUseCases
        viewModel.removeStepUseCase = tripModeUseCases
        viewModel.editPlanHourUseCase = tripModeUseCases
        viewModel.fetchStepAlternative = tripModeUseCases
        viewModel.fetchPlanAlternative = tripModeUseCases
        ...
        viewModel.tripObserverUseCase = tripModeUseCases
        ...
        viewModel.fetchOfferUseCase = offerUseCases
        viewModel.fetchOptInOfferUseCase = offerUseCases
        viewModel.observeOptInOfferUseCase = offerUseCases
        
        viewModel.exportItineraryUseCase = tripModeUseCases
        
        tripModeViewController = UIStoryboard.makeTripMode()
        tripModeViewController!.viewModel = viewModel
        viewModel.delegate = tripModeViewController!
        tripModeViewController!.delegate = self
...
extension TRPTripModeCoordinater: AddPlacesContainerViewControllerDelegate, AddPoiTableViewVCDelegate {
    ...
    private func makeAddPlacesContainer() -> UIViewController {
        ...
        for type in subCategory {
            let newSubView = makeAddPlacesListView(type: type.addPlaceType())
            viewController.addViewControllerInPagination(newSubView, type: type.addPlaceType())
        }
        
        if let tastes = selectedCity?.tastes, tastes.count > 0 {
            let mustTryTableView = makeMustTryTableView(tastes: selectedCity?.tastes ?? [] )
            viewController.addViewControllerInPagination(mustTryTableView, title: "Must Try")
        }
...
//        openYelpCoordinater(businessId: "rC5mIHMNF5C1Jtpb2obSkA", poi: poi, parentViewController: viewController)
    ...
    fileprivate func makePoiDetail(poi: TRPPoi,
        ...
        let viewModel = PoiDetailViewModel(place: poi, parentStep: parentStep)
...
extension TRPTripModeCoordinater {
    ...
    private func openExperienceCoordinater(nav: UINavigationController, tourId: Int? = nil) {
        ...
        experiencesCoordinater = TRPExperiencesCoordinater(navigationController: nav, cityName: cityName, destinationId: self.destinationId ?? "", tourId: tourId)
