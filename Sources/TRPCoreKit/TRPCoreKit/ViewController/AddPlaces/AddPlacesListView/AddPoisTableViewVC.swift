...
public protocol AddPoiTableViewVCContainerDelegate:AnyObject {
    func searchCleared()
}
...
class AddPoiTableViewVC: TRPBaseUIViewController {
    ...
    weak var containerDelegate: AddPoiTableViewVCContainerDelegate?
    ...
    @objc private func refreshData() {
        viewModel.updateContentMode(.recommendation)
        containerDelegate?.searchCleared()
    }
    ...
    fileprivate func showNoDataWarning() {
        if viewModel.contentMode == .recommendation  {
            tb.setEmptyText(TRPLanguagesController.shared.getLanguageValue(for: "trips.myTrips.itinerary.error.generatedStatusNegative1"))
        }else {
