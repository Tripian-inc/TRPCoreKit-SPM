...
public class AddPlacesContainerViewController: TRPBaseUIViewController {
    ...
    @objc func searchButtonPressed() {
        ...
        var currentType:AddPlaceTypes?
        if let indexItem = pagingViewController.state.currentPagingItem as? PagingIndexItem {
            let currentTitle = viewModel.getPagingTitle(index: indexItem.index)
            currentType = viewModel.getPlaceTypes(title: currentTitle)
        }
        delegate?.addPlaceContainerViewControllerOpenSearchView(navigationController, viewController: self, selectedType: currentType)
    }
    
    @objc func sortingButtonPressed() {
        openSortViewController()
    }
