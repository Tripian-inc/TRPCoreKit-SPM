...
public protocol MyTripTableViewVCDelegate: AnyObject {
    func myTripTableViewVCSelectedTrip(hash: String, city: TRPCity, arrival:String, departure:String, destinationId: String)
...
public class MyTripTableViewVC: TRPBaseUIViewController {
    ...
    public override func setupViews() {
        ...
        myTripRefreshControl!.attributedTitle = NSAttributedString(string: "Pull to refresh".toLocalized())
    ...
    fileprivate func showRemoveAlert(_ cell: TripCellModel) {
        //TOdo: -  fransızcası yapılacak 
        let optionMenu = UIAlertController(title: nil, message: "Are you sure you want to delete your trip?".toLocalized(), preferredStyle: .alert)
        
        let deleteAction = UIAlertAction(title: "Delete".toLocalized(), style: .default) { [weak self] _ in
            guard let strongSelf = self else {return}
            strongSelf.isBusyCell.append(cell.id)
            strongSelf.viewModel.removeTrip(tripHash: cell.hash)
        }
        
        let cancelAction = UIAlertAction(title: "Cancel".toLocalized(), style: .cancel) { [weak self] _ in
            guard let strongSelf = self else {return}
            strongSelf.unSelectedCell(id: cell.id)
        }
...
extension MyTripTableViewVC: MyTripTableViewDelegate {
    ...
    func setTableViewEmptyText() {
        ...
        if isDataLoaded == false {
            tb.setEmptyText("Loading your trips.".toLocalized())
        }else {
            if viewModel.getType() == .pastTrip {
                tb.setEmptyText("No past trips yet.".toLocalized())
            }else {
...
extension MyTripTableViewVC: UITableViewDelegate, UITableViewDataSource, EvrTableViewDelegate {
    ...
    private func createNewTripText() -> NSMutableAttributedString{
        ...
        let mainAttribute = NSMutableAttributedString(string: "No upcoming trips yet.\n Create a new trip".toLocalized(), attributes: typeAttributeStyle)
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if !isBusyCell.contains(info.id) {
            delegate?.myTripTableViewVCSelectedTrip(hash: info.hash,
                                                    ...
                                                    destinationId: info.profile.additionalData ?? "")
        }else {
