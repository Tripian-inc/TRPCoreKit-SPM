...
extension CreateTripStayShareViewModel {
    ...
    private func createData() {
        ...
        let personCell = CreateTripStayShareCellModel(title: TRPLanguagesController.shared.getLanguageValue(for: "trips.createNewTrip.form.travelerInfo.adults"), contentType: .adultNumber)
        let childCell = CreateTripStayShareCellModel(title: TRPLanguagesController.shared.getLanguageValue(for: "trips.createNewTrip.form.travelerInfo.children"), contentType: .childNumber)
        sections.append(CreateTripStayShareSectionModel(title: TRPLanguagesController.shared.getLanguageValue(for: "trips.createNewTrip.stepHeaders.travelerInfo"), cells: [personCell, childCell]))
//        sections.append(CreateTripStayShareSectionModel(title: "Children", cells: [childCell]))
        let accommodationCell = CreateTripStayShareCellModel(title: TRPLanguagesController.shared.getLanguageValue(for: "trips.createNewTrip.form.travelerInfo.accommodation.placeholder"), contentType: .accommodation)
        sections.append(CreateTripStayShareSectionModel(title: TRPLanguagesController.shared.getLanguageValue(for: "trips.createNewTrip.form.travelerInfo.accommodation.label"), cells: [accommodationCell], isRequired: true))
        let companionCell = CreateTripStayShareCellModel(title: TRPLanguagesController.shared.getLanguageValue(for: "add_companion"), contentType: .companion)
        let createCompanionCell = CreateTripStayShareCellModel(title: TRPLanguagesController.shared.getLanguageValue(for: "create_companion_profile"), contentType: .createCompanion)
        var sectionModel = CreateTripStayShareSectionModel(title: TRPLanguagesController.shared.getLanguageValue(for: "trips.createNewTrip.form.travelerInfo.companion.label"), cells: [companionCell, createCompanionCell])
        if getSelectedCompanions().count > 0 {
            ...
            sectionModel = CreateTripStayShareSectionModel(title:TRPLanguagesController.shared.getLanguageValue(for: "trips.createNewTrip.form.travelerInfo.companion.label") , cells: [companionCell, selectedCompanionsCell, createCompanionCell])
        }
...
extension CreateTripStayShareViewModel {
    ...
    public func canContinue() -> Bool {
        guard stayAddress != nil else {
            self.delegate?.viewModel(showMessage: TRPLanguagesController.shared.getLanguageValue(for: "select_start_location"), type: .error)
            return false
        }
        setTripProperties()
        return true
