...
//    private var showAlternativeBtn: TRPCirleButton?
    private var isSearchThisArea = true {
        didSet {
            if isSearchThisArea {
                searchAreaBtn?.title = "Search This Area"
            }else {
                searchAreaBtn?.title = "Clean Results"
            }
        }
    }
...
//        createTurnByTurnNavigation()
    ...
    @IBAction func userLocationPressed(_ sender: UIButton) {
        ...
        if permission.showAlert {
            EvrAlertView.showAlert(contentText: "You didn't give location permission.".toLocalized(), type: .warning)
        }
...
//        navigationItem.rightBarButtonItems = [myOffersBtn]
    ...
    fileprivate func showDailyList() {
        ...
        let days = viewModel.getDayList().map{ "Day".toLocalized() + " \($0.day) - \($0.date)" }
        ...
        let model = ActionModel("Change Day",nil, days, "Cancel")
    ...
    private func setupBlackTabbar() {
        ...
        blackTabbar.action = { [weak self] item in
            ...
            case .itinerary: ()
                ...
                stronSelf.delegate?.trpTripModeViewControllerOpenItinerary(stronSelf)
            case .experiences: ()
//                stronSelf.showMessage("Juniper Tours will be implemented soon.", type: .info)
                stronSelf.delegate?.trpTripModeViewControllerOpenExperience(stronSelf)
//            case .offer:
//                NotificationCenter.default.post(name: Notification.Name("TripianOpenOfferVC"), object: self, userInfo: ["vc": stronSelf, "cityId": stronSelf.viewModel.city.id])
            case .favourite:
                stronSelf.favoriteBtnPressed()
            case .search:
                stronSelf.delegate?.trpTripModeViewControllerOpenPlaces(stronSelf)
            }
        }
    }
}
...
extension TRPTripModeVC{
    @objc func changeTimePressed() {
        // showTimeDialog()
        let changeTimeAlert =  TimeAlert(title:  "My Travel", saveActionTitle: "Update".toLocalized(), cancelActionTitle: "Cancel".toLocalized(), applyButtonTitle: TRPAppearanceSettings.Common.ApplyButtonTitle,
                                         doneButtonTitle: TRPAppearanceSettings.Common.doneButtonTitle,
                                         startTimePlaceHolder: "From 9:00".toLocalized(),
                                         endTimePlaceHolder: "To 21:00".toLocalized())
...
extension TRPTripModeVC: TRPTripModeViewModelDelegate {
    ...
    public func viewModel(searchThisArea: [TRPPoi], isOffer: Bool) {
        ...
        if searchThisArea.count > 0 {
            ...
            let alertText = isOffer ? "No offers found." : "No result for this area."
            EvrAlertView.showAlert(contentText: alertText.toLocalized(), type: .info)
        }
...
extension TRPTripModeVC: ListOfRoutingPoisVCDelegate {
    ...
    public func listOfRoutingShowStepAlternative(step: TRPStep) {
        ...
        let alertName = "Alternative Locations to".toLocalized() + " \(step.poi.name)"
        ...
        let cancelButton = UIAlertAction(title: "Cancel".toLocalized(), style: UIAlertAction.Style.cancel) { (action) in}
...
extension TRPTripModeVC {
    ...
    func createSearchThisArea() {
        let title = TRPAppearanceSettings.TripModeMapView.searchThisAreaButtonTitle
    ...
    fileprivate func calculateSearchButtonPosition() -> CGRect {
        ...
        if TRPAppearanceSettings.TripModeMapView.searchThisAreaButtonTitle.count > 20 {
            width = 200
        }
    ...
    private func selectTypeOfSearchProperty() {
        ...
        let restaurant = SearchAreaTypes(name: TRPPoiCategory.restaurants.toString(), ids: [TRPPoiCategory.restaurants.getId()])
        let cafes = SearchAreaTypes(name: TRPPoiCategory.cafes.toString(), ids: [TRPPoiCategory.cafes.getId()])
        
        let attractionTypes = AddPlaceMenu.attractions.addPlaceType().subTypes
        let attraction = SearchAreaTypes(name: "Attractions".toLocalized(), ids:attractionTypes)
        
        let all = SearchAreaTypes(name: "All".toLocalized(), ids: [])
        ...
        let cancelButton = UIAlertAction(title: "Cancel", style: UIAlertAction.Style.cancel) { [weak self] (action) in
