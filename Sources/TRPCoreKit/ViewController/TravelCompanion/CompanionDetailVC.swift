...
//    }
    ...
    public override func viewWillAppear(_ animated: Bool) {
        ...
        if !viewModel.isUpdateType {
            titleLabel.text = "Create a Companion Profile".toLocalized()
            descLabel.text = "You can add your travel companion and share your plans".toLocalized()
            applyBtn.setTitle("Create New".toLocalized(), for: .normal)
            openKeyboard()
        } else {
...
extension CompanionDetailVC{
    ...
    public  func saveChanges() {
        guard !viewModel.getCompanionName().isEmpty else {
            companionDetailShowMessage("Name field is required.")
            return
        }
        guard let _ = viewModel.getSelectedTitle() else {
            companionDetailShowMessage("Select title for your companion.")
            return
        }
