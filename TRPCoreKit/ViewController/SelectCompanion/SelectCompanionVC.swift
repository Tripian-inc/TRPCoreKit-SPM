...
extension SelectCompanionVC{
    ...
    func updateApplyButtonTitle(){
        if self.viewModel.selectedItem.count == 0 && !viewModel.fromSDK && !viewModel.fromProfile{
            self.applyBtn.setTitle(TRPLanguagesController.shared.getContinueBtnText(), for: .normal)
        }else{
            self.applyBtn.setTitle(TRPLanguagesController.shared.getApplyBtnText(), for: .normal)
        }
    }
